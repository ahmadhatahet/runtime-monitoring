...
def run_parallel(dataset, postfix, lhl, flavor, eta, full, memory):
    ...
    neurons_dict = {"None": []}
    if flavor == "raw":
        neurons_dict["gte_mean"] = load_json(path_lhl / "neurons_gte_mean.json")
        neurons_dict["top_third"] = load_json(path_lhl / "neurons_top_third.json")
    ...
    with ProcessPoolExecutor(max_workers=min(num_cpus, 10)) as pool:
        ...
        results = [
            pool.submit(build_bdd_multi_etas, (
                    df_train.copy(),
                    df_test.copy(),
                    df_true.copy(),
                    df_eval.copy(),
