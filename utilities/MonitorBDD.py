...
class MonitorBDD:
    def __init__(self, num_classes, num_neurons, thld_1, thld_2=None, thld_3=None, subset_neurons=None, verbose=False):
        ...
        self.bdd = BDD()
        self.roots = { k:self.bdd.false for k in range(num_classes) }
        ...
        if subset_neurons is not None:
            self.subset_neurons = {int(k):v for k, v in subset_neurons.items()}
        else:
    ...
    def construct_pattern(self, cls, row):
        ...
        if self.subset_neurons is not None:
            ...
            vars = vars[self.subset_neurons[cls]]
            vars_not = vars_not[self.subset_neurons[cls]]
            if self.num_bits == 2:
    ...
    def check_one_pattern(self, cls, row):
        ...
        if (self.roots[cls] & expr) == self.bdd.false:
